# Helper functions for ChaCha20-Poly1305

import strutils

# Safe hex string to byte array conversion with validation
proc hexToBytes*(hexStr: string): seq[byte] =
    ...
    for i, c in cleanHex:
        if not c.isAlphaNumeric or not (c in {'0'..'9', 'a'..'f', 'A'..'F'}):
            raise newException(ValueError, "Invalid hex character at position " & $i)
    ...
    for i in 0..<result.len:
        ...
        try:
            ...
            raise newException(ValueError, "Invalid hex pair '" & hexPair & "' at position " & $(i*2))

# Safe byte array to hex string conversion
...
# Safe string to byte array conversion (UTF-8)
...
# Safe byte array to string conversion with validation
proc bytesToString*(data: openArray[byte]): string =
    ...
    for i in 0..<data.len:
        # Validate printable ASCII to prevent issues
        let b = data[i]
...
# Fully constant-time comparison for fixed-size arrays
...
# Secure memory clearing with DSE prevention
...
# Secure zero for fixed-size arrays using volatile writes
